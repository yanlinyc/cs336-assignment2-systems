...
def ddp_per_worker(rank: int, world_size: int, config: BenchmarkConfig):
    ...
    print(f"Rank {rank}: Starting warmpup phase, iterations: {config.num_warmups}")
    for _ in tqdm(range(config.num_warmups), desc=f"Rank {rank} warmup phase"):
        optimizer.zero_grad()
    ...
    for step in tqdm(range(num_iterations), desc=f"Rank {rank} training loop"):
        ...
        if config.debug and rank == 0:
            ...
            for (non_parallel_model_name, non_parallel_model_parameter), (
                ...
                assert torch.allclose(non_parallel_model_parameter, model_parameter, atol=1e-5), (
